...
import { getAllSessionsWithDetails } from '../../../shared/clinicDataStore';
...
const Timeline = () => {
    ...
    useEffect(() => {
        ...
        const interval = setInterval(() => setCurrentTimeMs(Date.now()), 60000); // update current time every minute
        return () => {
            window.removeEventListener('clinicDataUpdated', load);
    ...
    // Group sessions by hour string "HH:00"
    const grouped = sessions.reduce((acc, session) => {
        const h = session.startTime.split(':')[0];
    ...
    // Ensure we always have some hours to show if empty
                            ...
                            <div className="flex-1 flex flex-col sm:flex-row gap-2 border-t border-slate-100 pt-3 sm:pt-2 min-h-[60px]">
                                {hourSessions.length === 0 ? (
                                    ...
                                    hourSessions.map(session => {
                                        ...
                                        if (session.focus?.includes('OT')) {
                                            bgClass = "bg-teal-50 border-teal-200"; textClass = "text-teal-900"; subTextClass = "text-teal-700";
                                        } else if (session.focus?.includes('Speech') || session.focus?.includes('ST') || session.focus?.includes('SI')) {
                                            bgClass = "bg-indigo-50 border-indigo-200"; textClass = "text-indigo-900"; subTextClass = "text-indigo-700";
                                        }

                                        const h = parseInt(session.startTime.split(':')[0]);
                                        const m = parseInt(session.startTime.split(':')[1]);
                                        ...
                                        return (
                                            <div key={session.id} className={`${bgClass} border rounded-lg p-2 flex-1 relative ${isPast ? 'opacity-60' : ''} ${inProgress ? 'border-2 shadow-sm z-20' : ''}`}>
                                                ...
                                                <p className={`text-xs ${subTextClass}`}>{session.child?.name || 'Anak'} • {session.therapist?.name || 'Terapis'}</p>
                                            </div>
                                        )
                                    })
                                )}
