...
const STATUS_META = {
    pending_admin_review: {
        title: 'Butuh Review Admin',
        description: 'Request dari terapis yang perlu diputuskan admin.',
        icon: 'rate_review',
        className: 'bg-amber-50 text-amber-700 border-amber-200',
        panelClass: 'border-amber-200 bg-amber-50/50 dark:bg-amber-950/10',
    },
    approved_by_admin: {
        title: 'Menunggu Orang Tua',
        description: 'Sudah disetujui admin dan menunggu respons orang tua.',
        icon: 'hourglass_top',
        className: 'bg-sky-50 text-sky-700 border-sky-200',
        panelClass: 'border-sky-200 bg-sky-50/50 dark:bg-sky-950/10',
    },
    parent_confirmed: {
        title: 'Disetujui Orang Tua',
        description: 'Jadwal meeting sudah dikonfirmasi orang tua.',
        icon: 'check_circle',
        className: 'bg-emerald-50 text-emerald-700 border-emerald-200',
        panelClass: 'border-emerald-200 bg-emerald-50/50 dark:bg-emerald-950/10',
    },
    parent_declined: {
        title: 'Ditolak Orang Tua',
        description: 'Orang tua menolak jadwal yang diajukan.',
        icon: 'cancel',
        className: 'bg-red-50 text-red-700 border-red-200',
        panelClass: 'border-red-200 bg-red-50/50 dark:bg-red-950/10',
    },
    cancelled: {
        title: 'Dibatalkan Admin',
        description: 'Request dibatalkan atau tidak disetujui admin.',
        icon: 'event_busy',
        className: 'bg-slate-100 text-slate-700 border-slate-200',
        panelClass: 'border-slate-200 bg-slate-50 dark:bg-slate-800/30',
    },
};
...
export default function App({ mode = 'therapist' }) {
    ...
    return (
        <div className="bg-white dark:bg-slate-900 rounded-xl shadow-sm w-full overflow-hidden flex flex-col h-full border border-slate-200 dark:border-slate-800">
            <div className="px-4 sm:px-6 py-5 border-b border-slate-200 dark:border-slate-800 flex flex-col gap-3 sm:flex-row sm:justify-between sm:items-center shrink-0">
            ...
            <div className="grid grid-cols-1 xl:grid-cols-[420px_1fr] gap-0 flex-1 min-h-0">
                ...
                <section className="p-4 sm:p-6 overflow-y-auto">
                    {!loading && meetings.length > 0 && (
                        <div className="mb-5 rounded-2xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-900/40 p-3">
                            <div className="grid grid-cols-2 md:grid-cols-3 xl:grid-cols-6 gap-2">
                                <button
                                    type="button"
                                    onClick={() => setStatusFilter('all')}
                                    className={`rounded-xl border px-3 py-3 text-left transition ${statusFilter === 'all' ? 'border-slate-900 bg-white text-slate-900 shadow-sm dark:border-white dark:bg-slate-800 dark:text-white' : 'border-slate-200 bg-white/70 text-slate-600 hover:bg-white dark:border-slate-800 dark:bg-slate-800/60 dark:text-slate-300'}`}
                                >
                                    <span className="block text-xs font-black">Semua Status</span>
                                ...
                                {STATUS_GROUPS.map((status) => {
                                    ...
                                    return (
                                        <button
                                            ...
                                            className={`rounded-xl border px-3 py-3 text-left transition ${active ? `${meta.className} shadow-sm` : 'border-slate-200 bg-white/70 text-slate-600 hover:bg-white dark:border-slate-800 dark:bg-slate-800/60 dark:text-slate-300'}`}
                                        >
                    ...
                    {loading ? (
                        ...
                        <div className="h-full min-h-[240px] flex flex-col items-center justify-center text-center text-slate-500 gap-3">
                            ...
                            <p className="font-bold">{meetings.length === 0 ? 'Belum ada parent meeting.' : 'Tidak ada parent meeting di status ini.'}</p>
                        </div>
                    ) : (
                            ...
                            {visibleMeetings.map(meeting => (
                                <div key={meeting.id} className="rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50 dark:bg-slate-800/40 p-4">
                                    <div className="flex flex-col gap-3 sm:flex-row sm:items-start sm:justify-between">
                                        <div className="min-w-0">
                                            <p className="font-black text-slate-900 dark:text-white break-words">{meeting.childName}</p>
                                            <p className="text-sm text-slate-500 dark:text-slate-400 break-words">{meeting.parentName} - {meeting.therapistName}</p>
                                            <p className="text-xs text-slate-500 dark:text-slate-400 mt-1">{formatDate(meeting.date)} - {meeting.time} - {meeting.type}</p>
                                        </div>
                                        <span className={`shrink-0 self-start border rounded-full px-2.5 py-1 text-[10px] font-black ${statusClass(meeting.status)}`}>{STATUS_LABELS[meeting.status] || meeting.status}</span>
                                    </div>
                                    <div className="mt-3 rounded-lg bg-white dark:bg-slate-900/60 p-3 text-sm text-slate-600 dark:text-slate-300">
                                        ...
                                        {meeting.notes && <p className="mt-1 break-words">{meeting.notes}</p>}
                                        {meeting.reviewNote && <p className="mt-1 text-xs text-slate-500 break-words">Catatan admin: {meeting.reviewNote}</p>}
                                        {meeting.parentResponseNote && <p className="mt-1 text-xs text-slate-500 break-words">Catatan orang tua: {meeting.parentResponseNote}</p>}
                                    </div>
