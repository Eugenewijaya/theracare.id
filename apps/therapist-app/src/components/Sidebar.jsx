...
export default function Sidebar({ isOpen, onClose }) {
  ...
  useEffect(() => {
    const computeBadges = async () => {
      ...
      try {
        ...
        const scheduleCount = unread.filter(n => ['schedule_change', 'new_session', 'program_enrollment', 'substitute_confirmation', 'substitute_result'].includes(n.type)).length;
  ...
  const sidebarContent = (
    <aside className="w-[260px] flex-shrink-0 bg-slate-50 dark:bg-slate-900 border-r border-slate-200/80 dark:border-slate-800/80 flex flex-col h-full font-sans transition-colors duration-300">
      <div className="p-5 flex flex-col gap-6 flex-1 overflow-y-auto custom-scrollbar">
        ...
        {/* User Profile Hook */}
        <NavLink to="/performance" className="flex items-center gap-3.5 bg-white dark:bg-slate-950 p-3 rounded-2xl border border-slate-200/60 dark:border-slate-800/60 shadow-sm hover:border-teal-300 dark:hover:border-teal-700 transition-colors">
