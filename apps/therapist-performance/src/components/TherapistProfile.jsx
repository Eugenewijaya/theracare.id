...
const TherapistProfile = ({ user, onPhotoUpdate, uploading = false }) => {
    ...
    const handleFileChange = (e) => {
        ...
        if (file.size > MAX_PHOTO_SIZE) {
            setError('Ukuran foto maksimal 10 MB. Foto besar akan dikompres otomatis.');
            e.target.value = '';
            return;
        }
        ...
        <div className="flex p-4">
            <div className="flex w-full flex-col gap-4 sm:flex-row sm:justify-between sm:items-center">
                <div className="flex flex-col gap-4 sm:flex-row sm:items-center">
                    <button type="button" className="relative group cursor-pointer text-left w-32 shrink-0 disabled:cursor-wait" onClick={handlePhotoClick} disabled={uploading}>
                        {user?.avatar ? (
                            <div
                        ...
                        <div className="absolute inset-0 bg-black/40 rounded-full flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity">
                            <span className={`material-symbols-outlined text-white ${uploading ? 'animate-spin' : ''}`}>{uploading ? 'progress_activity' : 'photo_camera'}</span>
                        </div>
                        <input 
                            type="file" 
                    ...
                    <div className="flex flex-col justify-center">
                        ...
                        {uploading && <p className="mt-2 text-xs font-semibold text-teal-600 dark:text-teal-300">Mengompres dan mengunggah foto...</p>}
