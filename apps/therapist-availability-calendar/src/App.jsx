...
function getOffLabel(profile, date, hour) {
    const availability = getTherapistSlotAvailability(profile, date, `${String(hour).padStart(2, '0')}:00`, 60);
    if (!availability.known || availability.available) return '';
    if (availability.label.startsWith('Off mulai')) return `Off ${availability.window?.end || ''}`.trim();
    if (availability.label === 'Off hari ini') return 'Off';
                        ...
                        <div className="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm dark:border-slate-700 dark:bg-slate-800">
                            <div className="flex flex-col gap-4 lg:flex-row lg:items-center lg:justify-between">
                                ...
                                <div className="flex items-center gap-2">
                                        ...
                                        <div className="min-w-0 flex-1 text-center sm:min-w-[180px] sm:flex-none">
                                    ...
                                    <button onClick={handleToday} className="rounded-xl border border-slate-200 px-4 py-2 text-sm font-bold text-slate-600 transition-colors hover:bg-slate-50 dark:border-slate-700 dark:text-slate-300 dark:hover:bg-slate-700 whitespace-nowrap">
                                        Hari Ini
                                    </button>
                                </div>
                            </div>
                                        ...
                                        <div className="grid grid-cols-8 overflow-hidden rounded-b-2xl border-x border-b border-slate-200 dark:border-slate-700">
                                            ...
                                            {weekDates.map(date => {
                                                ...
                                                return (
                                                        ...
                                                        {HOURS.map(hour => {
                                                            const offLabel = getOffLabel(profile, date, hour);
                                                            return (
